    ...
    bool CheckPropertyType(ConditionalHideAttribute condHAtt, SerializedProperty sourcePropertyValue)
    {
        ...
        switch (sourcePropertyValue.propertyType)
        {
            case SerializedPropertyType.Boolean:
                return sourcePropertyValue.boolValue == condHAtt.boolValue;
            case SerializedPropertyType.Enum:
                if(condHAtt.enumIndices == null)
                    return sourcePropertyValue.enumValueIndex == condHAtt.enumIndex;
                else
                {
                    for(int i = 0; i < condHAtt.enumIndices.Length; ++i)
                    {
                        if (sourcePropertyValue.enumValueIndex == condHAtt.enumIndices[i])
                            return true;
                    }
                }
